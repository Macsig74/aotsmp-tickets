...
module.exports = {
  ...
  async execute(interaction) {
    ...
    modal.addComponents(
      ...
      new ActionRowBuilder().addComponents(
        new TextInputBuilder().setCustomId('plateforme').setLabel('Plateforme & version ✦ OBLIGATOIRE')
          .setStyle(TextInputStyle.Short).setPlaceholder('ex: Java 1.21.1 ou Bedrock 1.20.80')
          .setRequired(true).setMaxLength(100),
      ),
      new ActionRowBuilder().addComponents(
        new TextInputBuilder().setCustomId('code_achat').setLabel('Code de transaction / reçu ✦ OBLIGATOIRE')
          .setStyle(TextInputStyle.Short)
          .setPlaceholder('ex: TBX-XXXXXXXX — Renseigne une vraie valeur sinon aucune aide.')
          .setRequired(true).setMaxLength(100),
      ),
      new ActionRowBuilder().addComponents(
        new TextInputBuilder().setCustomId('probleme').setLabel('Décris ton problème ✦ OBLIGATOIRE')
